...
namespace Hsc.Foundation.Tests.Unit.Log
{
    ...
    class LogEntryBuilderTest
    {
        ...
        {
            LogEntryBuilder logEntryBuilder = new LogEntryBuilder(_logger);
        ...
        public void WithAllSetters()
        {
            ...
            var exception = new DuplicateWaitObjectException("foo");
            logEntryBuilder.WithMessage("foo {0}", "bar").AsFatal().WithEventId(42).WithCause("cause").WithResolution("resolution").WithException(exception).Write();
