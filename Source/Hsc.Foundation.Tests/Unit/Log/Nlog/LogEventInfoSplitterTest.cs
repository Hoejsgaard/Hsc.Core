...
namespace Hsc.Foundation.Tests.Unit.Log.Nlog
{
    ...
    {
        LogEventInfoSplitter _splitter;
        LogEventInfo _logEventInfo;
        ...
        private int MaxSize
        {
            get
            {
                return _splitter.MessageSizeLimit - _splitter.ReservedForFormatting;
            }
        }
        ...
        {
            _logEventInfo.Message = StringGenerator.BuildRandomString(MaxSize + 1);
            _logEventInfo.Level = NLog.LogLevel.Fatal;

            List<LogEventInfo> result = _splitter.GetEventFragments(_logEventInfo);

            Assert.That(result.Count, Is.EqualTo(2));
            Assert.That(result[0].Level, Is.EqualTo(NLog.LogLevel.Fatal));
            Assert.That(result[1].Level, Is.EqualTo(NLog.LogLevel.Fatal));
        }
